...
2. **查询用户音频路径**
   ...
   - 获取字段优先级：
     1. **优先使用 `tts_voice` 字段**（最终 AutoVoiceCloner 产物路径）
     2. **如果没有值，再使用 `cosy_voice` 字段**（CosyVoice V3 克隆音频路径）
     3. **如果 `cosy_voice` 和 `tts_voice` 都为空，但 `clean_input` 存在**：
        - 自动执行步骤 2（CosyVoice V3 声音克隆）和步骤 3（AutoVoiceCloner 最终声音克隆）
        - 执行完成后重新查询数据库，优先使用新生成的 `tts_voice`
        - 如果步骤 2 和步骤 3 执行后仍未生成有效音频文件，返回错误提示："角色音频克隆失败，步骤 2 和步骤 3 执行后仍未生成有效音频文件"
     4. **如果 `clean_input` 也不存在**，返回错误提示："角色音频录制不完整，请先完善角色音频录制后再生成"

   **注意**：步骤 2 和步骤 3 原本在创建角色时异步执行，现已移动到生成任务时按需同步执行，确保在需要使用音频时再执行克隆流程。
